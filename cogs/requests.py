...
class SelectMods(ui.Modal, title="Укажите модераторов (через запятую)"):
    ...
    def __init__(self, *, title: str = ..., timeout: float | None = None, custom_id: str = ..., bot: commands.Bot) -> None:
        self.bot = bot
        super().__init__(title=title, timeout=timeout, custom_id=custom_id)

    mods_list = ui.TextInput(label="Список модераторов", style=discord.TextStyle.paragraph, max_length=1000)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            cursor.execute(f"INSERT INTO helpers_sends_logs (helper_id, req_id, mod_name) VALUES {querry[:-2]}")
            db.commit()

            await interaction.response.edit_message(content="Твоё решение успешно занесено в БД! Не забывай, что отправить уровень модераторам нужно самостоятельно!", view=None, embed=None, delete_after=5)

            cursor.execute(f"INSERT INTO requests_logs (req_id, reviewer_id, reviewer_role) VALUES ({req_id}, {staff_id}, 2)")
            db.commit()
            db.close()

            emb = discord.Embed(title="Твой уровень был отправлен модераторам", description=f"Хелпер {interaction.user.name} отправил твой уровень модераторам!")
            emb.set_author(name=interaction.user.display_name, icon_url=interaction.user.avatar.url)
            emb.add_field(name="Отправлено следующим модераторам", value=', '.join(mod_list))
...
class FormForReq(ui.Modal, title="Отправка уровня хелперам"):
    ...
    level_id = ui.TextInput(label="ID уровня", style=discord.TextStyle.short, placeholder="87654321", max_length=9)
    level_video_link = ui.TextInput(label="Ссылка на видео (только YouTube)", style=discord.TextStyle.short, placeholder="https://www.youtube.com/watch?v=...")
    level_difficulty = ui.TextInput(label="Сложность (в звёздах)", style=discord.TextStyle.short, placeholder="10")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if match is None or lvl_link.find(' ') != -1:
                await interaction.response.send_message("<:no:1141747496813609011> Ошибка: Укажите ссылку на видео с YouTube", ephemeral=True)
                return
            ...
            try:
                ...
                await interaction.response.send_message("<:no:1141747496813609011> Ошибка: данный уровень уже был отправлен на ревью.", ephemeral=True)
                db.close()
                return
...
class RequestsCog(commands.Cog):
        ...
        try:
            ...
            match choose:
                case "ban":
                    ...
                    cursor.execute(f"INSERT INTO bans (requester_id, reason_id) VALUES ('{sender_discord}', 1)")
                    db.commit()

                    await interaction.response.edit_message(content="Репорт успешно закрыт", view=None, embed=None, delete_after=5)
                    

                    async for message in interaction.message.channel.history(limit=None):
                        try:
        ...
        try:
            ...
            match choose:
                case "clear":
                    ...
                    cursor.execute(f"DELETE FROM staff WHERE user_discord = '{staff_discord}'")
                    db.commit()
                    ...
                    emb.add_field(name="Почему так произошло?", value=f"Тебя снял с роли Мент {interaction.user.name}, так как ты злооупотреблял системой репортов по его мнению. Если это ошибка, обратись к администрации сервера", inline=False)
                ...
                case "ban":
                    ...
                    cursor.execute(f"UPDATE staff SET user_role = 0 WHERE user_discord = '{staff_discord}'")
                    db.commit()
                    ...
                    emb.add_field(name="Почему так произошло?", value=f"Тебя забанил Мент {interaction.user.name}, так как ты злооупотреблял системой репортов по его мнению. Если это ошибка, обратись к администрации сервера", inline=False)
    ...
    async def punish_reviewer(self, interaction: discord.Interaction):
        ...
        emb = discord.Embed(title="Укажите наказание", description="Выберите, как нужно наказать реквестера, если нужно вообще его наказывать", colour=discord.Colour.red())
        ...
        punishment_menu = ui.Select(
            ...
            options=[
                discord.SelectOption(label="Снять с роли", description="Убрать данного Ревьювера со своей роли", value=f"clear_{req_id}_{staff_discord}"),
                discord.SelectOption(label="Забанить", description="Заблокировать данного Ревьювера в системе, тем самым он не сможет стать Ревьювером или Хелпером", value=f"ban_{req_id}_{staff_discord}"),
                discord.SelectOption(label="Проигнорировать", description="Не применять никаких санкций в отношении этого Ревьювера", value=f"ignore_{req_id}_{staff_discord}")
            ])
    ...
    async def punish_requester(self, interaction: discord.Interaction):
        ...
        punishment_menu = ui.Select(
            placeholder="Выбрать наказание", 
            options=[
                discord.SelectOption(label="Запретить отправлять реквесты", description="Этот пользователь больше не сможет отпарвлять реквесты через /request", value=f"ban_{req_id}_{sender_discord}"),
                discord.SelectOption(label="Проигнорировать", description="Не применять никаких санкций в отношении этого пользователя", value=f"ignore_{req_id}_{sender_discord}")
            ])
    ...
    async def get_level_to_send(self, count, interaction, user_db) -> list:
        try:
            try:
                ...
                await interaction.response.send_message("Уровней для тебя пока что нет. Попробуй в другой раз", delete_after=5)
            ...
            await interaction.response.send_message("Похоже, что не нашлось уровня, который нуждается в твоей оценке", delete_after=5)
    ...
    async def generate_level_embed(self, level, level_data, original_guild_id) -> discord.Embed:
        try:
            ...
            if cp_count == 0:
                ...
                cp_text = f"У автора уровня **есть** КП ({cp_count}<:cp:1141823815081545839>)"
            else:
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if custom_id == "requestbutton":
            ...
            cursosr.execute(f"SELECT requester_id FROM bans WHERE requester_id = '{interaction.user.id}'")
            for x in cursosr:
                await interaction.response.send_message("<:no:1141747496813609011> Ошибка: ты не можешь отправлять реквесты, так как тебя заблокировали Менты.", ephemeral=True)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not (len(message.embeds) != 0 and message.embeds[0].title == "New rated level!"):
            return
        ...
        if level_data != []:
            try:
                ...
                new_embed.add_field(name="Наши поздравления!", value="Спасибо за твой реквест и искренне поздравляем с рейтом! Желаем успехов и больше удачных уровней")
                new_embed.set_footer(text="Команда РКГД", icon_url=message.guild.icon.url)
                new_embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/1140790207646552105/1141105250288287774/star.png")

                await sender.send(embed=new_embed)
            except Exception:
                print(f"Чё-то случилось в on_message. На инфу: {sender}, {new_embed}, {level_obj}")
            finally:
                cursor.execute(f"DELETE FROM requests_table WHERE req_id = {level_data[0]}")
                db.close()
                return
...
**• NSFW и NSFL уровни запрещены.**""", 
    ...
    @app_commands.command(name="review", description="Команда для ревьюверов: позволяет оценивать уровни")
    async def review(self, interaction: discord.Interaction):
        try:
            ...
            if len(user_db) == 0:
                await interaction.response.send_message("Ты не можешь использовать эту команду, так как не являешься ревьювером", delete_after=5)
                db.close()
                return
            ...
            async def finish_callback(interaction: discord.Interaction):
                await interaction.response.edit_message(content="Завершаем...", embed=None, view=None, delete_after=5)
            ...
            async def report_callback(interaction: discord.Interaction):
                try:
                    ...
                    await interaction.response.edit_message(content="<:yes:1141747509899841637> Твой репорт был отправлен. Скоро менты его рассмотрят.\nПродолжишь смотреть уровни?", embed=None, view=view)
                    view.clear_items()

                    dif = enums.DifficultyCalculator.get_difficulty_by_stars(requested_stars)
                    level_data = parser.get_parsed_level_data(level_id)
                    emb = discord.Embed(title="Новый репорт!", description=f"Ревьювер __{interaction.user.name}__ создал новый репорт на уровень __{level_data[0]}__", colour=discord.Colour.red())
                    emb.set_author(name=f"Request ID: {req_id}", icon_url=interaction.user.avatar.url)
                    emb.add_field(
                        name="Об уровне", 
                    ...
                    punish_requester = ui.Button(label="Наказать реквестера", style=discord.ButtonStyle.red, custom_id=f"{sender_id}_{req_id}_req")
                    punish_reviewer = ui.Button(label="Ложный репорт", style=discord.ButtonStyle.grey, custom_id=f"{interaction.user.id}_{req_id}_rep")
    ...
    @app_commands.command(name="rate", description="Команда для хелперов: позволяет оценивать уровни")
    async def rate(self, interaction: discord.Interaction):
        try:
            ...
            if count == []:
                await interaction.followup.send("Пока что, уровней на оценку нет", delete_after=5)
                db.close()
                return
            ...
            async def check_callback(interaction: discord.Interaction):
                try:
                    ...
                    await interaction.response.send_message(f"Уровень был отправлен следующим модераторам:\n{mods}", ephemeral=True)
            ...
            async def get_data_callback(interaction: discord.Interaction):
                try:
                    ...
                    cursor.execute(f"SELECT level_id, video_link, requested_stars FROM requests_table WHERE req_id = {req_id}")
                    level_id = ""
                    level_link = ""
                    level_rs = 0
                    for x in cursor:
