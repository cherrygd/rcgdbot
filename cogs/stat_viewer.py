...
class ModalForLevelInof(ui.Modal, title="Укажите DBID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if len(result) == 0:
                await interaction.response.send_message(f"{EH.NO.value} **Ошибка**: уровень с DBID *{dbid}* не найден среди Ваших уровней", ephemeral=True)
                db.close()
                return
            ...
            rating_text += EH.YES.value + ' ' + (EH.GREEN.value * (round(result[3] / rc * 10))) + (EH.RED.value * (round(result[4] / rc * 10))) + ' ' + EH.NO.value
    ...
    async def stats(self, interaction: discord.Interaction):
        try:
            ...
            if user_role in {1, 2}:
                ...
                desc = f'{EH.CP.value} ревьювером' if user_role == 1 else f'{EH.STAR.value} хелпером'
                emb = discord.Embed(
                ...
                try:
                    ...
                    result = cursor.fetchall()
                    is_banned_stf_text = f"{EH.REP.value} Заблокирован" if len(result) != 0 else f"{EH.YES.value} Отсутствует"
                    ...
                    show_all_levels_button     = ui.Button(emoji=EH.HO.value, style=discord.ButtonStyle.red, custom_id=f"all_{user.id}")
                    show_specific_level_button = ui.Button(emoji=EH.DOWNLOAD.value, style=discord.ButtonStyle.green, custom_id=f"spec_{user.id}")
